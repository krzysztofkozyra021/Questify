        ...
        <div class="flex items-stretch gap-2 pr-2 md:pr-4">
          ...
          <div class="flex-1 py-2 cursor-pointer hover:bg-stone-50 transition-colors duration-200 min-w-0" @click="openEditDailyModal(daily)">
            <div class="flex flex-wrap items-center gap-1 md:gap-2">
              ...
              >
                <span v-if="daily.difficulty.icon" class="mr-1" v-html="daily.difficulty.icon" />
                {{ trans(daily.difficulty.name) }}
              </span>
            </div>
            <div class="flex flex-wrap items-center gap-1 md:gap-2 mt-1">
              <span v-if="daily.experience_reward && !daily.is_completed" class="text-xs font-bold text-stone-600 bg-amber-100 px-1.5 py-0.5 rounded flex items-center whitespace-nowrap shrink-0">
              ...
              <span v-if="daily.difficulty && getDailyEnergyPenalty(daily) > 0 && !daily.is_completed" class="text-xs font-bold text-stone-600 bg-blue-100 px-1.5 py-0.5 rounded flex items-center whitespace-nowrap shrink-0">
