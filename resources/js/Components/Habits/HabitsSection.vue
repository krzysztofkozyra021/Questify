  ...
  <section class="bg-stone-100 rounded-xl shadow-lg p-2 sm:p-4 md:p-6 lg:p-8 flex flex-col min-h-[200px] sm:min-h-[300px] md:min-h-[400px] lg:min-h-[500px]">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center mb-3 sm:mb-4">
      ...
      <div class="flex flex-row gap-2 sm:gap-3 mt-2 sm:mt-0 sm:ml-auto">
        <button class="text-xs sm:text-sm font-medium text-stone-600 hover:text-amber-600 transition-colors duration-200 whitespace-nowrap" 
                @click="showAllHabits"
        >
          {{ trans('All') }}
        </button>
        <button :class="{ 'text-amber-600 border-b-2 border-amber-600': showWeakHabits }" 
                ...
                @click="showWeakHabits = !showWeakHabits; showStrongHabits = false"
        >
          {{ trans('Weak') }}
        </button>
        <button :class="{ 'text-amber-600 border-b-2 border-amber-600': showStrongHabits }" 
                class="text-xs sm:text-sm font-medium text-stone-600 hover:text-amber-600 transition-colors duration-200 whitespace-nowrap" 
                @click="showStrongHabits = !showStrongHabits; showWeakHabits = false"
        >
          {{ trans('Strong') }}
        </button>
      </div>
    ...
    <ul class="flex-1 space-y-2 overflow-y-auto pr-1">  
      <li v-for="habit in habitsFiltered" :key="habit.id" class="bg-white rounded-lg shadow">
        <div class="flex items-stretch gap-2">
          ...
          >
            <button class="size-4 md:size-6 flex items-center justify-center text-white font-bold text-sm md:text-base hover:scale-110 transition-all duration-200 ease-in-out" 
          ...
          <div class="flex-1 py-2 cursor-pointer hover:bg-stone-50 transition-colors duration-200 min-w-0" @click="openEditHabitModal(habit)">
            <div class="flex flex-wrap items-center gap-1 md:gap-2">
              ...
              <span v-if="habit.difficulty" class="ml-1 md:ml-2 px-1.5 md:px-2 py-0.5 rounded text-xs font-bold text-center whitespace-nowrap shrink-0" 
                    :style="{ backgroundColor: habit.difficulty.color || '#57534e', color: '#ffffff' }"
              >
                <span v-if="habit.difficulty.icon" class="mr-1" v-html="habit.difficulty.icon" />
                {{ trans(habit.difficulty.name) }}
              </span>
            </div>
            <div class="flex flex-wrap items-center gap-1 md:gap-2 mt-2">
              <span v-if="habit.completed_count !== undefined" class="text-xs font-bold text-stone-600 whitespace-nowrap shrink-0">
                +{{ habit.completed_count }} | -{{ habit.not_completed_count }}
              </span>
              <span v-if="habit.experience_reward" class="text-xs font-bold text-stone-600 bg-amber-100 px-1.5 py-0.5 rounded flex items-center whitespace-nowrap shrink-0">
              ...
              <span v-if="habit.difficulty && calculateHabitHealthPenalty(habit) > 0" class="text-xs font-bold text-stone-600 bg-red-100 px-1.5 py-0.5 rounded flex items-center whitespace-nowrap shrink-0">
              ...
              <span v-if="habit.difficulty && calculateHabitEnergyPenalty(habit) > 0" class="text-xs font-bold text-stone-600 bg-blue-100 px-1.5 py-0.5 rounded flex items-center whitespace-nowrap shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-3 text-blue-500 mr-1 shrink-0" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div class="flex items-center justify-center p-2 md:px-3 rounded-r-lg shrink-0"
               :class="{'bg-lime-500' : habit.completed_count > habit.not_completed_count,
                        'bg-amber-500' : habit.completed_count === habit.not_completed_count,
                        'bg-red-500' : habit.completed_count < habit.not_completed_count
               }"
